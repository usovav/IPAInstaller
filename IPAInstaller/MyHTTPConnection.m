...
- (NSString *)createBrowseableIndex:(NSString *)path
{
	...
	[outdata appendFormat:@"<title>IPAInstaller</title>"];
	...
	[outdata appendFormat:@"<h1>IPAs</h1>", server.name];
    [outdata appendString:@"<bq>The following files are hosted live from the IPAInstaller's Docs folder.</bq>"];
    [outdata appendString:@"<p>"];
	[outdata appendFormat:@"<a href=\"..\">..</a><br />\n"];
    for (NSString *fname in array)
    { 
        if([fname isEqualToString:@"metas"])
        {
            continue;
        }else{
	...
	if ([self supportsPOST:path withSize:0])
	{
		...
		[outdata appendString:@"<label>Upload .ipa"];
